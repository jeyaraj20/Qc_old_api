...
module.exports = {
    ...
    setExamTaken: async (req, res, next) => {
        try {
            ...
            if (count > 0) {
                console.log(rows[0]);
                await db.SchoolExamtakenlistQc.create({
                    stud_id: req.user.id,
    ...
    getExamTaken: async (req, res, next) => {
        try {
            ...
            const [examtaken] = await db.sequelize.query(
                `select * from tbl__schoolexamtaken_list_qc as a 
            ...
            const [examquestion] = await db.sequelize.query(
                ...
                left join tbl__schoolexam_result_qc as b on a.exq_id=b.quest_id and b.exam_id=?
                and b.taken_id=?
    ...
    setExamResult: async (req, res, next) => {
        try {
            ...
            const examresultdata = await db.SchoolExamresultQc.findAndCountAll({
                where: {
                    exam_id: exam_id,
                    stud_id: req.user.id,
                    taken_id: taken_id,
            ...
            if (examresultdata.rows.length == 0) {
                ...
                await db.SchoolExamresultQc.create({
                    school_id: req.user.schoolId,
                    stud_id: req.user.id,
                    taken_id: taken_id,
                    exam_id: exam_id,
                ...
                await db.SchoolExamresultQc.update(
                    {
                        attend_ans: ansval,
                        review_val: reveval,
                        lastupdate: moment(Date.now()).format("YYYY-MM-DD HH:mm:ss"),
    ...
    saveExamResult: async (req, res, next) => {
        try {
            ...
            // Total attend
            const totalAttend = await db.SchoolExamresultQc.count({ where: { taken_id } }).catch((err) => {
                throw createError.InternalServerError(err.message);
            });

            const totalAttendwithanswer = await db.SchoolExamresultQc.count({ where: { taken_id, attend_ans: { [Op.ne]: 0 } } }).catch((err) => {
                throw createError.InternalServerError(err.message);
            });
            ...
            const crtansw = await db.sequelize
                .query(
                    `SELECT COUNT(*) AS crtansw FROM tbl__schoolexam_result_qc WHERE taken_id = ?
            ...
            const wrongansw = await db.sequelize
                .query(
                    `SELECT COUNT(*) AS wrongansw FROM tbl__schoolexam_result_qc WHERE taken_id = ?
            ...
            await db.SchoolExamtakenlistQc.update(
                {
                    ...data,
    ...
    getResult: async (req, res, next) => {
        try {
            ...
            const result = await db.SchoolExamtakenlistQc.findOne({
                where: {
                    taken_id: taken_id,
                    exam_status: "C",
                },
            ...
            const rank = await db.SchoolExamtakenlistQc.count({
                where: {
            ...
            const totalAttend = await db.SchoolExamtakenlistQc.count({
                where: {
                    exam_id: result.exam_id,
                    exam_status: "C",
                },
            ...
            if (result.exam_type == "C") {
                ...
                const [totalanswered] = await db.sequelize
                    .query(
                        `select COUNT(*) as totalattend, sect_id from tbl__schoolexam_result_qc  where taken_id = ? and attend_ans!=0 and exam_id = ? GROUP BY sect_id`,
                ...
                const [totalseen] = await db.sequelize
                    .query(
                        `select COUNT(*) as totalattend, sect_id from tbl__schoolexam_result_qc  where taken_id = ?  and exam_id = ? GROUP BY sect_id`,
                        { replacements: [taken_id, result.exam_id] }
                    )
                    .catch((err) => {
                        throw createError.InternalServerError(err.message);
                ...
                const [crtanswer] = await db.sequelize
                    .query(
                        `SELECT COUNT(*) AS crtansw, sect_id FROM tbl__schoolexam_result_qc WHERE taken_id = ?
    ...
    getSchoolById: async (req, res, next) => {
        try {
            ...
            if (schoolid == 0) throw createError.BadRequest();
    ...
    getAllExamCount: async (req, res, next) => {
        try {
            ...
            count = await db.SchoolExams.count({
                where: { exam_type: type, exam_status: status, exam_sub_sub: exa_cat_id, schoolid: req.user.id },
            }).catch((err) => {
                throw createError.InternalServerError(err.message);
            });
            res.send({ count });
