...
module.exports = {
    ...
    SendResultSms: (mobileNo, user_name, obtainmark, exam_name) => {
        // let sms_text = `Dear ${user_name}, you have scored ${obtainmark} marks in ${exam_name} Question Cloud`
        let sms_text = `Dear ${user_name}, You have scored ${obtainmark} marks in ${exam_name} Question Cloud. -QNSCLD`
        ...
        let smsApiUri = `http://fastsms.expressad.in/api/v1/send-sms?api-key=K2TyCySMXMrmlPxpluT5ndwSDc8ekEav4DYMvOdK&sender-id=QNSCLD&sms-type=2&mobile=${mobileNo}&te_id=1207162911343716631&message=${sms_text}`;
    ...
    SendOtpSms: (mobileNo, otpNo,name) => {
        let sms_text = `Dear ${name}, ${otpNo} is the OTP to verify your QuestionCloud account. -QNSCLD`;
        ...
        //let smsApiUri = `http://bhashsms.com/api/sendmsg.php?user=questioncloud&pass=1234&sender=QNSCLD&phone=${mobileNo}&text=${sms_text}&priority=ndnd&stype=normal`;
        // let smsApiUri = `http://fastsms.expressad.in/api/v1/send-sms?api-key=K2TyCySMXMrmlPxpluT5ndwSDc8ekEav4DYMvOdK&sender-id=QNSCLD&sms-type=1&route=1&mobile=${mobileNo}&message=${sms_text}`;
        let smsApiUri = `http://fastsms.expressad.in/api/v1/send-sms?api-key=K2TyCySMXMrmlPxpluT5ndwSDc8ekEav4DYMvOdK&sender-id=QNSCLD&sms-type=2&mobile=${mobileNo}&te_id=1207162911323349485&message=${sms_text}`;
        http.get(smsApiUri, (resp) => {
            let data = "";
            // A chunk of data has been recieved.
            resp.on("data", (chunk) => {
    ...
    SendForgotPasswordSms: (username, password, mobileNo) => {
        let sms_text = `Hi ${username}, ${password} is your password for Question Cloud account. - QNSCLD`;
        //let smsApiUri = `http://bhashsms.com/api/sendmsg.php?user=questioncloud&pass=1234&sender=QNSCLD&phone=${mobileNo}&text=${sms_text}&priority=ndnd&stype=normal`;
        let smsApiUri = `http://fastsms.expressad.in/api/v1/send-sms?api-key=K2TyCySMXMrmlPxpluT5ndwSDc8ekEav4DYMvOdK&sender-id=QNSCLD&sms-type=2&mobile=${mobileNo}&te_id=1207162194667618020&message=${sms_text}`;
